...
public:
	...
	struct Ref
	{
		...
		Ref(const char * buf = 0)
		{
			if(buf)	CALL_MEMBER_FN(this, ctor)(buf);
			else data = 0;
		}
		~Ref(){ if(data) CALL_MEMBER_FN(this, Release)(); }
...
// Modified by HDT
// Now it look like std::string more
class BSFixedString : public StringCache::Ref
{
public:
	BSFixedString():StringCache::Ref(""){}
	
	BSFixedString(const char* rhs);
	BSFixedString(const std::string& rhs);
	BSFixedString(const BSFixedString& rhs);
	BSFixedString(BSFixedString&& rhs);

	BSFixedString& operator =(const BSFixedString& rhs);
	BSFixedString& operator =(BSFixedString&& rhs);
	
	BSFixedString operator+(const BSFixedString& rhs) const;

	bool operator ==(const BSFixedString& rhs) const;
	bool operator ==(const char* rhs) const;
	bool operator ==(const std::string& rhs) const;
	inline bool operator !=(const BSFixedString& rhs) const{ return !(*this == rhs); }
	
	MEMBER_FN_PREFIX(BSFixedString);
	DEFINE_MEMBER_FN(Length, UINT, 0x00A50AC0);

	inline UINT Length(){ return CALL_MEMBER_FN(this, Length)(); }

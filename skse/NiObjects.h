...
{
public:
	NiCloningProcess() { CALL_MEMBER_FN(this, ctor)(0x3F800000); }
	MEMBER_FN_PREFIX(NiCloningProcess);
	DEFINE_MEMBER_FN(ctor, void, 0x0046DF40, UINT32);

	NiObject* from;		//unverified
	NiObject* to;		//unverified
	UINT32 unk0[0x0F];
	char m_cAppendChar;
	UINT32 unk1[0x03];
};

STATIC_ASSERT(sizeof(NiCloningProcess) == 0x54);
STATIC_ASSERT(offsetof(NiCloningProcess, m_cAppendChar) == 0x44);
...
public:
	...
	DEFINE_MEMBER_FN(Clone, NiObject *, 0x00AAFC00, NiCloningProcess* kCloning);
};
...
public:
	...
	DEFINE_MEMBER_FN(FindExtraData, NiExtraData*, 0x00AB45F0, const BSFixedString&);
};
...
public:
	...
	virtual NiAVObject *	GetObjectByName(const BSFixedString& name);	// BSFixedString? alternatively BSFixedString is a typedef of a netimmerse type
	virtual void	SetSelectiveUpdateFlags(bool * selectiveUpdate, bool selectiveUpdateTransforms, bool * rigid);
	virtual void	UpdateDownwardPass(ControllerUpdateContext * ctx, UINT* dirtFlag = 0);
	virtual void	UpdateSelectedDownwardPass(ControllerUpdateContext * ctx, UINT* dirtFlag = 0);
	virtual void	UpdateRigidDownwardPass(ControllerUpdateContext * ctx, UINT* dirtFlag = 0);
	virtual void	UpdateWorldBound(void);
	virtual void	UpdateWorldData(ControllerUpdateContext * ctx);		// 0x00ab53e0, call 0x00ab5370
	...
	NiNode		* m_parent;			// 18
	NiCollisionObject	* m_collision;			// 1C
...
public:
	...
	{
		UInt32				unk00;			// 00
		NiNode				* m_meshRoot;	// 04
		NiGeometry			* m_triShape;	// 08
		UInt32				unk0C;			// 0C
